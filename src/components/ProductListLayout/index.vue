      ...
      <el-col :span="16">
        <div class="product-card">
          ...
          <div class="seriveFilter">
            <el-checkbox-group>
              <el-checkbox v-for="value in serviceMap" :key="value.value">{{
                value.label
              }}</el-checkbox>
            </el-checkbox-group>
          </div>
...
const priceName = computed(() => {
  const field: any = priceSortMap.find(
    (item: any) => item.key === priceSort.value
  );
  return field?.name;
});
